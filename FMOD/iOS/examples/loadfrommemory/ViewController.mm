...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    */    
    ...
    loadFileIntoMemory(buffer, &filebuff, &length);
    
    memset(&exinfo, 0, sizeof(FMOD_CREATESOUNDEXINFO));
    exinfo.cbsize = sizeof(FMOD_CREATESOUNDEXINFO);
    exinfo.length = length;
    
    result = system->createSound((const char *)filebuff, FMOD_SOFTWARE | FMOD_OPENMEMORY, &exinfo, &sound1);
    ERRCHECK(result);
    
    result = sound1->setMode(FMOD_LOOP_OFF);    /* drumloop.wav has embedded loop points which automatically makes looping turn on, */
    ERRCHECK(result);                           /* so turn it off here.  We could have also just put FMOD_LOOP_OFF in the above CreateSound call. */
    
    free(filebuff); // Don't need the original memory any more.  Note!  If loading as a stream, the memory must stay active so do not free it!
    ...
    result = system->createSound((const char *)filebuff, FMOD_SOFTWARE | FMOD_OPENMEMORY, &exinfo, &sound2);
    ERRCHECK(result);
    ...
    result = system->createSound((const char *)filebuff, FMOD_SOFTWARE | FMOD_OPENMEMORY, &exinfo, &sound3);
    ERRCHECK(result);
