...
@synthesize detectedrate;
@synthesize detectednote;
...
- (void)timerUpdate:(NSTimer *)timer 
{
    ...
    */
    ...
    detectedrate.text   = [NSString stringWithFormat:@"%7.1f -> %7.1f hz", dominanthz, ((float)bin + 0.99f) * BINSIZE]; 
    detectednote.text   = [NSString stringWithFormat:@"%-3s (%7.1f hz)", note[dominantnote], notefreq[dominantnote]]; 
}


- (void)dealloc 
{   
	[detectedrate release];
  	[detectednote release];
	[super dealloc];
}
