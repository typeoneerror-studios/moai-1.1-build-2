...
namespace {
  ...
  class ReadInProgress {
   ...
   private:
    void ReadMore() {
      PRINTF("ReadMore: size=%"PRId64", offset_=%"PRId64"\n",
             data_->size(), offset_);
      bool chunking = true;
    ...
    void BytesWereRead(int64_t bytes_read) {
      PRINTF("BytesWereRead: bytes_read=%"PRId64", size=%"PRId64
             ", offset_=%"PRId64"\n",
  ...
  class WriteInProgress {
   ...
   private:
    void WriteMore() {
      PRINTF("WriteMore: size=%"PRId64", offset_=%"PRId64"\n",
             data_->size(), offset_);
    ...
    void FileLengthWasSet(int32_t result) {
      PRINTF("FileLengthWasSet result=%"PRId32"\n", result);
      // TODO(dmichael): Fails on Mac Canary, Linux Chrome 14.
      //CHECK(result == 0);
      BytesWereWritten(0);
    }
    void BytesWereWritten(int64_t bytes_written) {
      PRINTF("BytesWereWritten: bytes_written=%"PRId64", size=%"PRId64
             ", offset_=%"PRId64"\n",
