...
ios_schemes=( "libmoai-ios" "libmoai-ios-3rdparty" "libmoai-osx-fmod-ex" "libmoai-ios-facebook" "libmoai-ios-luaext" "libmoai-ios-tapjoy" "libmoai-ios-untz" "libmoai-ios-zlcore" )
    ...
    for config in $configurations; do
        for sdk in $sdks; do
            for scheme in $schemes; do
                ...
                xcodebuild -configuration $config -workspace $src/libmoai.xcodeproj/project.xcworkspace -scheme $scheme -sdk $sdk clean CONFIGURATION_BUILD_DIR=/tmp/$platform/$job/libmoai/$scheme/$sdk/$config
                echo "Done"
            done
        done
